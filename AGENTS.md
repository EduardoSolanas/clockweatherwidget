...
- When doing git operations, lint, tests, or verification, use a cheaper sub-agent (such as `flash_lite`) to run or cross-check the command workflow whenever the active tool policy permits sub-agents. If sub-agents are unavailable or blocked by higher-priority instructions, state that explicitly and run the checks directly.
