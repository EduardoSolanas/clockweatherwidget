...
| 2 | City/weather snapshot identity | PARTIAL | Weather-owned label/coordinates; existing-row location saved after successful fetch | Optional-section reuse after movement; no single location+weather transaction |
...
| 5 | Refresh ownership and coverage | PARTIAL | Provider preference uses distinctUntilChanged | Widget hard-codes seven days; broader request coalescing separate |
...
| 9 | Request volume and rendering cost | OPEN | Source paths identified | Measure requests early; address fallback flash; optimize only with evidence |
...
1. **Baseline measurement (9), before behavior changes.** Record the source revision and dirty diff, settings, cache age/coverage, locations and refresh triggers. Count enqueue attempts, actual worker runs, cache hits and HTTP requests separately, including endpoint pages, retries and fallback. Cover three- and seven-day forecasts with fresh and stale cache. Keep this a bounded baseline, not a billing investigation that blocks correctness work. Repeat the same scenarios after each fix; do not infer downloads or cost from scheduled opportunities alone.
2. **Widget coverage mismatch (5).** Read the saved forecast setting and align the freshness target with supported coverage and the widget's minimum needs. Add a real-data regression for a three-day selection: no permanently unsatisfied seven-day freshness target. A stale decision here can enqueue redundant work; it does not prove every enqueue downloads, because the worker rechecks freshness.
3. **Optional-cache relocation guard (2).** Before passing cache to either selected or fallback provider in refreshAndPersist, compare the requested coordinates with the persisted weather snapshot coordinates. On significant movement, pass null so old AQ/pollen cannot be reused under the new city. This is a bounded correctness fix using existing metadata, independent of a location transaction or provenance redesign. Missing or invalid snapshot coordinates must also disallow reuse: migrated rows have nullable fields, while getWeatherData can substitute location-row coordinates, so comparing only the mapped domain location is insufficient. Preserve the old persisted/displayed snapshot if core fetching fails. Use real persistence regressions for same-location reuse, significant movement, unknown migrated coordinates and optional-request failure after movement.
4. **Widget fallback flash.** Follow the home-screen section below: preserve useful content during routine refresh and failure; verify first placement, recovery and both launcher rendering paths. This is directly relevant to the user's current widget concern.
5. **Resume freshness (4).** Add one freshness-gated check on ON_RESUME while retaining immediate cache display and avoiding duplicate startup downloads.
6. **Remaining correctness/policy gaps (6, 7, 8).** Finish cancellation propagation, define whether no-widget settings may schedule, and separate follow-device intent from evidence of a real fix. Recheck the precise path immediately before implementation; current evidence is summarized at each finding.
7. **Measured improvements.** Consider broader request coalescing or renderer optimizations only with concrete benefit and regression coverage. A full location+weather transaction is not yet implemented; assess its remaining failure scenarios separately from the save-order fix.
...
Status: proposed work, based on another source inspection after the fixes above. Those fixes do not establish flicker-free launcher rendering or guarantee fresh weather whenever the user looks at the widget.
