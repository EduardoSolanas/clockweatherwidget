...
class RefreshWeatherUseCaseTest {
    ...
    fun `forceRefresh passes forecastDays 14 to repository`() = runTest {
        coJustRun { repository.forceRefreshWeatherData(location, 14, HourlyScope.EXTENDED) }

        useCase.forceRefresh(location, forecastDays = 14)

        coVerify(exactly = 1) { repository.forceRefreshWeatherData(location, 14, HourlyScope.EXTENDED) }
    }

    @Test
    fun `ensureFresh defaults to 7 forecast days when not specified`() = runTest {
        coJustRun { repository.ensureFreshWeatherData(location, 7, hourlyScope = HourlyScope.EXTENDED) }

        useCase.ensureFresh(location)

        coVerify(exactly = 1) { repository.ensureFreshWeatherData(location, 7, hourlyScope = HourlyScope.EXTENDED) }
    }

    @Test
    fun `forceRefresh passes forecastDays 1 to repository`() = runTest {
        coJustRun { repository.forceRefreshWeatherData(location, 1, HourlyScope.EXTENDED) }

        useCase.forceRefresh(location, forecastDays = 1)

        coVerify(exactly = 1) { repository.forceRefreshWeatherData(location, 1, HourlyScope.EXTENDED) }
    }
}
