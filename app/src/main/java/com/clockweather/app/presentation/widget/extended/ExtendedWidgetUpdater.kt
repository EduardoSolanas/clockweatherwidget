...
class ExtendedWidgetUpdater(
    ...
    fun updateWidget(appWidgetId: Int) {
        val views = RemoteViews(context.packageName, R.layout.widget_extended)
        ...
        scope.launch {
            try {
                ...
                val showToday = dataStore.data.map { prefs ->
                    prefs[booleanPreferencesKey("show_today_extended")] ?: false
                }.first()
                ...
                WidgetDataBinder.bindWeeklyForecastRows(context, views, weather, TemperatureUnit.CELSIUS)
